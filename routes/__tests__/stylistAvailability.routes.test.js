...
app.use("/stylist-availability", routes);
...
describe("stylistAvailability.routes", () => {
  ...
  it("GET / returns rows", async () => {
    ...
    const res = await request(app).get("/stylist-availability");
  ...
  it("GET /:id returns 404 if missing", async () => {
    ...
    const res = await request(app).get("/stylist-availability/99");
  ...
  it("POST / creates row", async () => {
    ...
    const res = await request(app).post("/stylist-availability").send(payload);
  ...
  it("PUT /:id updates row", async () => {
    ...
    const res = await request(app).put("/stylist-availability/1").send({ day_of_week: 2 });
    expect(res.status).toBe(200);
  ...
  it("DELETE /:id deletes row", async () => {
    ...
    const res = await request(app).delete("/stylist-availability/1");
