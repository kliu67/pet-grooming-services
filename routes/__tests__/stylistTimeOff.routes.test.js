...
app.use("/stylist-time-off", routes);
...
describe("stylistTimeOff.routes", () => {
  ...
  it("GET / returns rows", async () => {
    ...
    const res = await request(app).get("/stylist-time-off");
  ...
  it("GET /:id returns 404 if missing", async () => {
    ...
    const res = await request(app).get("/stylist-time-off/99");
  ...
  it("POST / creates row", async () => {
    ...
    const res = await request(app).post("/stylist-time-off").send(payload);
  ...
  it("PUT /:id updates row", async () => {
    ...
    const res = await request(app).put("/stylist-time-off/1").send({ reason: "Updated" });
    expect(res.status).toBe(200);
  ...
  it("DELETE /:id deletes row", async () => {
    ...
    const res = await request(app).delete("/stylist-time-off/1");
