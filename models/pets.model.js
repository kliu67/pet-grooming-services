...
export async function findByOwner(ownerId) {
  ...
  const { rows } = await pool.query(
    `
    SELECT p.id, p.name, p.uuid, p.created_at, p.updated_at,
...
export async function findById(id) {
  ...
  const { rows } = await pool.query(
    `
    SELECT id, name, species, owner, uuid, created_at, updated_at
    FROM pets
    WHERE id = $1
    `,
    [numericId]
 ...
 * Create pet
 */
export async function create({ name, species, owner }) {
  ...
  try {
    const { rows } = await pool.query(
      ...
      VALUES ($1, $2, $3)
      RETURNING id, name, species, owner, uuid, created_at, updated_at
      `,
      [normalizedName, speciesId, ownerId]
    );
